...
void mpcinc_mtx_multiply_mtx_vec(real_t pout[], const real_t pmtx[],
        const real_t pvec[],
        const uint32_t rows,
        const uint32_t cols)
{
 ...
 * pmtx has size (rows x columns) */
void mpcinc_mtx_scale(real_t pout[], const real_t pmtx[],
        ...
        const uint32_t cols)
{
  ...
  mpcinc_mtx_multiply_mtx_vec(pvecab, pmtxa, pvecb, rows, cols);
    for (k = 0; k < rows; k++) {
        pvecc[k] += pvecab[k];
    }
    return;
...
void mpcinc_mtx_saturate_vec(real_t pvec[], const real_t plower[],
        const real_t pupper[],
        const uint32_t rows)
{
...
void mpcinc_mtx_max_vec_zero(real_t pmax[], const uint32_t rows)
{
    uint32_t i; /* vector index (row number) */
    real_t zero = 0.;
    for (i = 0; i < rows; i++) {
...
void mpcinc_mtx_min_vec_zero(real_t pmin[], const uint32_t rows)
{
    uint32_t i; /* vector index (row number) */
    real_t zero = 0.;
    for (i = 0; i < rows; i++) {
...
void mpcinc_mtx_transpose(real_t * mtxout, const real_t * mtxin,
        const uint32_t rows, const uint32_t cols)
{
    uint32_t i; /* row number */
    uint32_t j; /* column number */
...
void mpcinc_mtx_shift_sequence(real_t outseq[], const real_t seq[],
