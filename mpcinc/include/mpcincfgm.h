...
}; /**< Configuration parameters of the MPCINC algorithm. */

struct mpcinc_fgm {
struct mpcinc_fgm_conf *conf;  /**< Algorithm configuration data. */
...
extern void mpcinc_fgm_solve_problem(const struct mpcinc_fgm *fgm);

extern void mpcinc_fgm_warm_start(const struct mpcinc_fgm *fgm);

extern void mpcinc_fgm_minimize_qp_iteration(const struct mpcinc_fgm *fgm,
                real_t u[], real_t u_old[], real_t w[], const real_t gradoL[]);

extern void mpcinc_fgm_compute_grad_over_L(const struct mpcinc_fgm *fgm,
                real_t gradoL[], const real_t w[]);

extern void mpcinc_compute_gxoL(struct mpcinc_fgm *fgm, const real_t x[]);
